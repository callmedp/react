...
class TrendingCourseAPI(APIView):
    ...
    def get(self, request, format=None):
        ...
        """
        t_courses = []
        NUM_COURSE_TO_SELECT = 3

        conversion_ratio_product = Product.objects.filter(product_class__slug__in=settings.COURSE_SLUG).order_by('-buy_count')[:NUM_COURSE_TO_SELECT]
        revenue_per_mile = Product.objects.filter(product_class__slug__in=settings.COURSE_SLUG).annotate(
            revenue=(F('buy_count') * F('cp_page_view')) * 1000 / F('cp_page_view'))\
            .exclude(id__in=list(conversion_ratio_product.values_list('id', flat=True))).order_by('-revenue')[:NUM_COURSE_TO_SELECT]

        product_pks = list(conversion_ratio_product.values_list('id', flat=True)) + list(revenue_per_mile.values_list('id', flat=True))
