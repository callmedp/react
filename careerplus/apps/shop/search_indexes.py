...
class ProductIndex(indexes.SearchIndex, indexes.Indexable):
            ...
            if obj.is_course:
                ...
                for pv in pop:
                    pop_list.append({
                        ...
                        'inr_price': float(pv.inr_price),
                        'rating': pv.get_ratings(),
                        'avg_rating': pv.get_avg_ratings(),
                        'fake_inr_price': float(pv.fake_inr_price),
                        'usd_price': float(pv.usd_price),
                        'fake_usd_price': float(pv.fake_usd_price),
                        'aed_price': float(pv.aed_price),
    ...
    def prepare_pRT(self, obj):
        review = Review.objects.only('id', 'status', 'title', 'user_email', 'user_id', 'user_name','content',
                                     'average_rating','created').filter(object_id=obj.id,status__in=[0,1])
    ...
    def prepare_pSec(self, obj):
        ...
        for section in sections:
            ...
            new_dict.update({'image': section.get_image(),'heading':section.name,'data':data_list,
                             'slug':section.section_type_text()})
