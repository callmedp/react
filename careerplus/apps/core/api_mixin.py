...
class ShineToken(object):
    def get_client_token(self):
        try:
            ...
            client_data = {'key': settings.CLIENT_ACCESS_KEY,
                           'secret': settings.CLIENT_ACCESS_SECRET}
            client_access_resp = requests.post(
                client_access_url, data=client_data, headers=headers)
            client_access_resp_json = client_access_resp.json()
            if client_access_resp.status_code == 201:
                cache.set('shine_client_access_token', client_access_resp_json.get(
                    'access_token'), timeout=None)
                return client_access_resp_json.get('access_token', None)
        except Exception as e:
            logging.getLogger('error_log').error(
                'error in getting client token%s' % str(e))
        return None
    ...
    def get_access_token(self):
        try:
            ...
            user_data = {"email": settings.SHINE_API_USER,
                         "password": settings.SHINE_API_USER_PWD}
            user_access_resp = requests.post(
                user_access_url, data=user_data, headers=headers)
            user_access_resp_json = user_access_resp.json()
            if user_access_resp.status_code == 201:
                cache.set('shine_user_access_token', user_access_resp_json.get(
                    'access_token'), timeout=None)
                return user_access_resp_json.get('access_token', None)
        except Exception as e:
            logging.getLogger('error_log').error(
                'error in accessing token %s' % str(e))
        return None
    ...
    def get_api_headers(self, token=None):
        try:
            ...
            logging.getLogger('error_log').error(
                'error in getting header %s' % str(e))
        return None
...
class ShineCandidateDetail(ShineToken):
    ...
    def get_shine_id(self, email=None, headers=None):
        ...
        try:
            ...
            if email:
                ...
                solr_query = '?fl=id&wt=json&qt=edismax&rows=1&q=sEm:{}'.format(
                    email)
                response = requests.get('{}{}'.format(
                    candidate_solr_url, solr_query))
            ...
            if email and headers:
                ...
                shine_id_response = requests.get(
                    shine_id_url, headers=headers, timeout=settings.SHINE_API_TIMEOUT)
            ...
            logging.getLogger('error_log').error(
                'unable to get shine id  %s' % str(e))
        return None
        ...
        try:
            if shine_id:
                ...
                detail_url = settings.SHINE_SITE + \
                    "/api/v2/candidate-profiles/" + shine_id + "/?format=json"
                detail_response = requests.get(
                    detail_url, headers=headers, timeout=settings.SHINE_API_TIMEOUT)
                ...
                logging.getLogger('error_log').error(
                    'unable to get candidate details {} {}'.format(email, shine_id))
                return {}
            ...
            logging.getLogger('error_log').error(
                'unable to get candidate details%s' % str(e))
    ...
    def get_candidate_public_detail(self, email=None, shine_id=None):
        ...
        if not shine_id and email:
            ...
            logging.getLogger('error_log').error(
                "Email ID or shine_id required for profile")
            return
        detail_url = settings.SHINE_SITE + \
            "/api/v2/candidate-public-profiles/" + shine_id + "/?format=json"
        try:
            detail_response = requests.get(
                detail_url, headers=headers, timeout=settings.SHINE_API_TIMEOUT)
            if detail_response.status_code == 401:
            ...
            logging.getLogger('error_log').error(
                'unable to get detail response %s' % str(e))
        return
    ...
    def get_status_detail(self, email=None, shine_id=None, token=None):
        try:
            if shine_id:
                headers = self.get_api_headers()
                status_url = "{}/api/v2/candidate/{}/status/?format=json".format(
                ...
                shine_id = self.get_shine_id(email=email, headers=headers)
                if shine_id:
                    status_url = settings.SHINE_SITE + "/api/v2/candidate/" + \
            ...
            logging.getLogger('error_log').error(
                'unable to get status details %s' % str(e))
        return None
            ...
            if candidate_id and resume_id:
                if not headers:
                    ...
                    api_url = settings.SHINE_SITE + '/api/v2/candidate/' + \
                        candidate_id + '/resumefiles/' + resume_id + '/'
...
class FeatureProfileUpdate(ShineToken):
    ...
    def update_feature_profile(self, candidate_id=None, data={}, headers=None):
        try:
            ...
            logging.getLogger('error_log').error(
                'unable to update profile details %s' % str(e))
        return False
...
class ShineCertificateUpdate(ShineToken):
    ...
    def update_shine_certificate_data(self, candidate_id=None, data={}, headers=None):
        try:
            if candidate_id:
                ...
                if data and headers:
                    ...
                    certification_response = requests.post(
                        certificate_api_url, data=data, headers=headers)
            ...
            logging.getLogger('error_log').error(
                'unable to update certificate %s' % str(e))
        return False, None
...
class AdServerShine(object):
    ...
    def encode(self, email, mobile, timestamp):
        inp_str = '{key}|{email}|{mobile}|{timestamp}'.format(
            **{'key': settings.MOBILE_ADSERVER_ENCODE_KEY, 'email': email, 'timestamp': timestamp, 'mobile': mobile})
...
class AcrossShine(object):
    ...
    def encode(self, email):
        inp_str = '{email}|{key}'.format(
            **{'key': settings.ACROSS_ENCODE_KEY, 'email': email})
...
class CrmApiMixin(object):
    ...
    def get_api_headers(self):
        try:
            ...
            logging.getLogger('error_log').error(
                'unable to return header %s' % str(e))
        return None

    def create_lead_by_api(self, data_dict={}):
        try:
            headers = self.get_api_headers()
            lead_create_api = settings.SHINECPCRM_DICT.get(
            ...
            logging.getLogger('error_log').error(
                'error in creating lead by api %s' % str(e))
...
class AmcatApiMixin(object):
    ...
    def get_headers(self, data, api_url, method_type):
        ...
        api_signature = self.get_api_signature(
            token, secret, data, api_url, method_type)
        headers = {
    ...
    def get_all_certiticate_data(self, data):
        ...
        if response.status_code == 200:
            ...
            logging.getLogger('info_log').info(
                "amcat import certificate for email {} api response:{}".format(
                    str(data), jsonrsp)
            )
            return True, jsonrsp
        else:
    ...
    def get_auto_login_url(self, data):
        ...
        if resp.status_code == 200:
            ...
            logging.getLogger('info_log').info(
                "AutoLogin url for data %s successfully retrieved" % (
                    str(data))
            )
            return autologin_url
        else:
            logging.getLogger('error_log').error(
                "Failed fetching autologin for data:- %s , Error:- %s" % (
                    str(data), str(resp.content))
            )
            return False
...
class NeoApiMixin(object):
    ...
    def board_user_on_neo(self, email, data_dict):
        ...
        if user_id:
            ...
            if resp.status_code == 200:
                ...
                logging.getLogger('error_log').error(
                    'Unable to board user because {}'.format(resp.content))
        else:
            logging.getLogger('error_log').error(
                'Unable to board user {} as user id not found'.format(email))
    ...
    def get_student_account_type(self, email):
        ...
        if student_sso:
            account_type = student_sso.get('acl', {}).get(
                'account', {}).get('type', '')
            if not account_type:
                return
            return account_type
