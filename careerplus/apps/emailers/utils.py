...
def get_featured_profile_data_for_candidate(candidate_id, curr_order_item, feature=False):
    others_feature_profile_items_sub_type_flow = ShineProfileData.objects.all()
    ...
    other_featured_items = Order.objects.filter(
        ...
        orderitems__product__sub_type_flow__in=others_feature_profile_items_sub_type_flow,
        orderitems__oi_status=28
    ).exclude(order=curr_order_item.order)
