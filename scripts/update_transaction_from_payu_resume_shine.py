...
def create_params (var,command):
    ...
    data = {}
    data.update({'var1': var, 'key': settings.PAYU_INFO1.get('merchant_key'), 'command': command})
...
if __name__ == "__main__":
    ...
    for interval in time_intervals_to_check:
        ...
        all_initiated_transactions = PaymentTxn.objects.filter(status__in=[0,2,3,4,5],\
                payment_mode=13,created__gte=sdt,order__site=2)
        ...
        try:
            res = requests.post(settings.PAYU_INFO1.get('web_api_url'),
                                params_data)
