...
if __name__=="__main__":
    ...
    for interval in time_intervals_to_check:
        ...
        all_initiated_transactions = PaymentTxn.objects.filter(status__in=[0,2,3,4,5],\
                payment_mode=5,created__gte=sdt,created__lte=edt,order__site=0)
        ...
        for txn in all_initiated_transactions:
            ...
            try:
                ...
                logging.getLogger('error_log').error(\
                    "Unable to decrypt CCAvenue reponse {} {} {}".format(str(e),txn.txn,decrypted_text))

            if not decrypted_dict:
                logging.getLogger('error_log').error(\
                    "CCAvenue Unable to decrypt {} {}".format(decrypted_dict,txn.txn))
                continue
