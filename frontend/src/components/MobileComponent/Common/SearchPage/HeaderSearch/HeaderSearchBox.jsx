...
const HeaderSearchBox = (props) =>{
    ...
    useEffect(()=>{
        crossSearch && document.getElementById("transcript").focus();
    }, [])
    ...
    return(
        <>
            <div className="m-top-search-header">
                {
                    crossSearch && <figure className="micon-close-white mr-20" onClick={()=>setShowSearchPage(false)}></figure>
                }
                <form id="searchForm" className="form-inline w-100 ml-auto" onSubmit={handleSubmit(submitData)}>
                    ...
                    <input className="m-search-input" type="search" onChange={e => setSearchTerm(e.target.value)} onFocus={()=>setShowResults(true)} 
                        placeholder = {placeholder} name="query" id="transcript" aria-label="Search" ref={register({required: true})} autoComplete="off" />
