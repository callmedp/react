...
const SkillPage = (props) => {
  ...
  const meta_tags = useSelector((store) =>
    store.skillBanner.meta ? store.skillBanner.meta : ""
  );
  ...
  try {
    ...
    try {
      ...
      if (!(window && window.config && window.config.isServerRendered)) {
        ...
        new Promise((resolve, reject) =>
          dispatch(
            fetchDomainJobs({ payload: { id: pageId }, resolve, reject })
          )
        );
        new Promise((resolve, reject) =>
          dispatch(
    ...
    if (query["t_id"]) {
      ...
      if (
        tracking_data["prod_id"] != pageId &&
        tracking_data["product_tracking_mapping_id"] == "10"
      ) {
        removeTrackingInfo();
      }
    }
  ...
  return (
    <div>
      ...
      <StickyNav
        hasFaq={hasFaq}
        hasLearnerStories={hasLearnerStories}
